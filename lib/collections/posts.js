...
Posts.after.insert(function(userId, doc) {
  ...
  Forums.update({_id: doc.forumId}, {
    ...
    $set: {
      ...
      lastPostThreadSlug: Threads.findOne({_id: doc.threadId}).slug
    }
  });
  ...
  if(forumLastPost) {
    Forums.update({_id: doc.forumId}, {$inc: {postCount: -1},
      $set: {
        ...
        lastPostThreadSlug: Threads.findOne({_id: forumLastPost.threadId}).slug
      }
    });
  } else {
    Forums.update({_id: doc.forumId}, {$inc: {postCount: -1},
