...
Meteor.methods({
  postInsert: function(postProperties) {
    ...
    if(postId) {
      ...
      Threads.update(thread, {$inc: {postCount: 1}, $set: {lastPostDate: date, lastPostAuthor: user.username}});
      Forums.update({_id: forumId}, {
        $inc: {postCount: 1},
        $set: {
          lastPostDate: date,
          lastPostAuthor: user.username,
          lastPostThreadId: thread._id,
          lastPostThreadTitle: thread.title
        }
      });
